...
import {
  AppBar, Toolbar, Typography, IconButton, InputBase, Box, Slide,
  Paper, Checkbox, FormControlLabel, Divider, CircularProgress, Button, List, ListItem, ListItemText
} from "@mui/material";
...
const Header = ({ onToggleSidebar }) => {
  ...
  const [results, setResults] = useState({ nhaKhoa: [], benhNhan: [], donHang: [] });
  ...
  return (
    <>
      <AppBar position="fixed" sx={{ zIndex: 1201, background: "#1DA1F2" }}>
        <Toolbar sx={{ display: "flex", justifyContent: "space-between", gap: 2, minHeight: { xs: 64, sm: 70 }, px: { xs: 1, sm: 2 }, position: "relative" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, minWidth: 0, flexShrink: 0 }}>
            ...
            <Typography variant="h6" onClick={() => navigate("/")} sx={{ cursor: "pointer", fontWeight: 600, fontSize: { xs: "14px", sm: "16px", md: "20px" } }}>
              TẤN DENTAL
            </Typography>
          </Box>
          ...
          <Box
            ...
            sx={{
              position: { xs: searchOpen ? "absolute" : "relative", sm: "relative" },
              ...
              backgroundColor: { xs: searchOpen ? "#1DA1F2" : "transparent", sm: "transparent" },
          ...
          >
            ...
            {/* Input chính */}
            <Box sx={{ display: "flex", alignItems: "center", backgroundColor: "white", borderRadius: "999px", px: 2, py: 0.5, width: "100%" }}>
              ...
              />
              {isSearching && <CircularProgress size={20} sx={{ color: "#1DA1F2" }} />}
            </Box>
            ...
            {isDropdownOpen && (
              <Paper sx={{
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", p: 1, bgcolor: "#f8fafc", borderBottom: "1px solid #e2e8f0", alignItems: "center" }}>
                  {SEARCH_OPTIONS.map((opt) => (
                    <FormControlLabel
                      key={opt.id}
                      control={<Checkbox size="small" checked={selectedTypes.includes(opt.id)} onChange={() => handleTypeToggle(opt.id)} />}
                      label={<Typography variant="body2" sx={{ fontSize: '13px' }}>{opt.label}</Typography>}
                      sx={{ mr: 1.5, mb: 0 }}
                    />
                  ))}
                  <Box sx={{ flexGrow: 1 }} />
                ...
                <Box sx={{ p: 1 }}>
                  {!searchQuery ? (
                    <Typography variant="body2" color="textSecondary" sx={{ p: 2, textAlign: "center" }}>Gõ để bắt đầu tìm kiếm...</Typography>
                  ) : (
                    <>
                      {/* 1. ĐƠN HÀNG */}
                      {results.donHang.length > 0 && (
                        <Box mb={2}>
                          <Typography variant="subtitle2" sx={{ px: 2, py: 1, bgcolor: "#fef3c7", color: "#d97706", borderRadius: 1, fontWeight: "bold" }}>
                            ĐƠN HÀNG
                          </Typography>
                          <List dense>
                            {results.donHang.map((dh) => (
                              <ListItem button key={dh._id} onClick={() => { setIsDropdownOpen(false); setSearchOpen(false); navigate(`/donhang/${dh._id}`); }}>
                                <ListItemText
                                  primary={`Mã: ${dh.maDonHang}`}
                                  secondary={
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', mt: 0.5 }}>
                                      <Typography variant="caption" color="textSecondary">
                                        Nha khoa: {dh.nhaKhoa?.hoVaTen || dh.nhaKhoa?.tenGiaoDich || 'Trống'}
                                      </Typography>
                                      <Typography variant="caption" color="textSecondary">
                                        Bệnh nhân: {dh.benhNhan?.hoVaTen || 'Trống'}
                                      </Typography>
                                    </Box>
                                  }
                                />
                      ...
                      {results.benhNhan.length > 0 && (
                        <Box mb={2}>
                          <Typography variant="subtitle2" sx={{ px: 2, py: 1, bgcolor: "#fce7f3", color: "#db2777", borderRadius: 1, fontWeight: "bold" }}>
                            BỆNH NHÂN
                          </Typography>
                          <List dense>
                            {results.benhNhan.map((bn) => (
                              <ListItem button key={bn._id} onClick={() => { setIsDropdownOpen(false); setSearchOpen(false); navigate(`/benhnhan/${bn._id}`); }}>
                                <ListItemText primary={bn.hoVaTen} secondary={bn.namSinh || ''} />
                              </ListItem>
                            ))}
                          </List>
                        </Box>
                      ...
                      {results.nhaKhoa.length > 0 && (
                        <Box mb={2}>
                          <Typography variant="subtitle2" sx={{ px: 2, py: 1, bgcolor: "#e0f2fe", color: "#0284c7", borderRadius: 1, fontWeight: "bold" }}>
                            KHÁCH HÀNG (NHA KHOA)
                          </Typography>
                          <List dense>
                            {results.nhaKhoa.map((nk) => (
                              <ListItem button key={nk._id} onClick={() => { setIsDropdownOpen(false); setSearchOpen(false); navigate(`/nhakhoa/${nk._id}`); }}>
                                <ListItemText primary={nk.hoVaTen || nk.tenGiaoDich} secondary={nk.soDienThoai} />
                              </ListItem>
                            ))}
                          </List>
                        </Box>
                      ...
                      {!isSearching && results.nhaKhoa.length === 0 && results.benhNhan.length === 0 && results.donHang.length === 0 && (
                        <Typography variant="body2" color="textSecondary" sx={{ p: 2, textAlign: "center" }}>Không tìm thấy kết quả nào.</Typography>
                      )}
                    </>
                  )}
                </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 0.5, sm: 1.5 }, flexShrink: 0 }}>
            ...
            {isAuthenticated ? (
              <><QuickAddMenu /><IconButton color="inherit"><NotificationsIcon /></IconButton><HeaderUser /></>
            ) : (<LoginModal />)}
          </Box>
