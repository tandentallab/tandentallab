...
export default function NhapXuatTable() {
    ...
    const { phieuNhapKhos, loading: loadingNhap, loadingMore: loadingMoreNhap, hasMore: nhapHasMore } = useSelector((state) => state.phieuNhapKho);
    const { phieuXuatKhos, loading: loadingXuat, loadingMore: loadingMoreXuat, hasMore: xuatHasMore, boPhanList } = useSelector((state) => state.phieuXuatKho);
    ...
    function buildNhapParams(page = 1) {
        ...
        const nhapTrangThai = selectedTrangThai.filter((t) => ["Chưa nhận", "Đã nhận"].includes(t));
        return {
            limit: 20,
            page,
            ...dateRange,
    ...
    function buildXuatParams(page = 1) {
        const dateRange = monthToDateRange(selectedMonth);
        const xuatTrangThai = selectedTrangThai.filter((t) => ["Chưa xuất", "Đã xuất"].includes(t));
        return {
            limit: 20,
            page,
            ...dateRange,
    ...
    const vatLieuNhap = useMemo(() => aggregateVatLieu(phieuNhapKhos), [phieuNhapKhos]);
    const vatLieuXuat = useMemo(() => aggregateVatLieu(phieuXuatKhos), [phieuXuatKhos]);
    ...
    return (
        <div>
            ...
            <FilterToolbar
                ...
                selectedTrangThai={selectedTrangThai} onToggleTrangThai={handleToggleTrangThai}
            ...
            {/* Modals tạo mới */}
            <NhapKhoModal open={showNhapModal} onClose={handleNhapModalClose} editData={null} />
            <XuatKhoModal open={showXuatModal} onClose={handleXuatModalClose} editData={null} />
