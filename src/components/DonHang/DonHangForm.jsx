      ...
      <div className="bg-gray-100 px-4 sm:px-6 py-3 flex flex-wrap justify-between items-center gap-2 border-t z-10 shadow-lg shrink-0">
        <div className="flex flex-wrap gap-2">
          ...
          {isEditMode && validWarranties.length > 0 && (
            <button
              onClick={() => {
                const qrCode = validWarranties[0].maQR || validWarranties[0].maBaoHanh || "";
                window.open(`https://tan-dental-frontend-snmb.vercel.app/warranty/?qrcode=${qrCode}`, "_blank");
              }}
              className="bg-teal-500 hover:bg-teal-600 text-white px-4 py-1.5 rounded text-sm flex items-center gap-1"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-4 h-4">
