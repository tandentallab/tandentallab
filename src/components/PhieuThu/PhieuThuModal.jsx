...
export default function PhieuThuModal({ open, onClose, onSuccess }) {
    ...
    const tongThuTien = useMemo(
        () => Object.values(selectedHDs).reduce((s, v) => s + (v.soTienThanhToan || 0), 0),
        [selectedHDs]
    );

    const tongConNo = useMemo(() => {
        if (!hoaDonChuaThanhToan.length) return 0;
        return hoaDonChuaThanhToan.reduce((s, hd) => s + (hd.conLai || 0), 0) - tongThuTien;
    }, [hoaDonChuaThanhToan, tongThuTien]);

    const handleSubmit = async () => {
        setSubmitError("");
        if (!selectedNhaKhoa) { setSubmitError("Vui lòng chọn khách hàng."); return; }
        const entries = Object.entries(selectedHDs);
        if (!entries.length) { setSubmitError("Vui lòng chọn ít nhất một hóa đơn."); return; }
        ...
        try {
            ...
            setSubmitError(typeof err === "string" ? err : "Có lỗi xảy ra khi tạo phiếu thu.");
        }
    };
    ...
    const tenKhach = selectedNhaKhoaObj?.hoVaTen || selectedNhaKhoaObj?.tenGiaoDich || "";
    const address = selectedNhaKhoaObj
        ? [selectedNhaKhoaObj.diaChiCuThe, selectedNhaKhoaObj.quanHuyen, selectedNhaKhoaObj.tinh].filter(Boolean).join(", ")
        : "";
        ...
        <div className="fixed inset-0 z-[1300] flex items-start justify-center pt-4 pb-4 overflow-y-auto">
            ...
            <div className="relative bg-gray-50 rounded-2xl shadow-2xl w-full max-w-5xl mx-4 flex flex-col" style={{ maxHeight: "calc(100vh - 32px)" }}>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                    ...
                    {submitError && (
                        <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-2 text-sm">{submitError}</div>
                    )}
                    {submitSuccess && (
                        <div className="bg-green-50 border border-green-200 text-green-700 rounded-lg px-4 py-2 text-sm">Tạo phiếu thu thành công!</div>
                    )}
                        ...
                        <div>
                            ...
                            <div className="relative" ref={searchRef}>
                                <div className="flex items-center border-b-2 border-gray-300 focus-within:border-[#29b6f6] pb-1 gap-2">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => { setSearch(e.target.value); setShowDropdown(true); if (!e.target.value) handleClearNhaKhoa(); }}
                                        className="flex-1 text-sm font-semibold text-gray-800 bg-transparent outline-none placeholder-gray-400"
                                    />
                                    {selectedNhaKhoa && (
                                        <button onClick={handleClearNhaKhoa} className="text-gray-400 hover:text-gray-600 text-lg leading-none shrink-0">&times;</button>
                                    )}
                                </div>
                                {showDropdown && filteredNhaKhoa.length > 0 && (
                                    <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-52 overflow-y-auto">
                                        {filteredNhaKhoa.map((nk) => (
                                            <div key={nk._id} onMouseDown={() => handleSelectNhaKhoa(nk)}
                                                className="px-4 py-2.5 hover:bg-blue-50 cursor-pointer">
                                                <p className="text-sm font-medium text-gray-800">{nk.hoVaTen || nk.tenGiaoDich}</p>
                                                {nk.moTa && <p className="text-xs text-gray-400 truncate">{nk.moTa}</p>}
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            {selectedNhaKhoaObj && (
                                <div className="mt-3 bg-[#e3f2fd] rounded-xl px-4 py-3 space-y-1 text-sm">
                                    ...
                                    <div className="flex gap-2"><span className="text-gray-500 w-20 shrink-0">Điện thoại:</span><span className="text-gray-500">{selectedNhaKhoaObj.soDienThoai || ""}</span></div>
                                    <div className="flex gap-2"><span className="text-gray-500 w-20 shrink-0">Mô tả:</span><span className="text-gray-700">{selectedNhaKhoaObj.moTa || ""}</span></div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="rounded-xl overflow-hidden border border-gray-200 bg-white">
                        {loadingHoaDon ? (
                            ...
                            <div className="text-center py-8 text-gray-300 text-sm">Chọn khách hàng để xem hóa đơn</div>
                    ...
                    <div className="grid grid-cols-2 gap-8 items-start">
                        ...
                        <div className="flex items-start justify-end pt-1">
                            <div className="text-right">
                                ...
                                <span className="text-xl font-bold text-gray-900">{fmt(Math.max(0, tongConNo))}</span>
                            </div>
                        </div>
                    </div>
