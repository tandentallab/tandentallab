    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle className="bg-[#0ea5e9] text-white flex justify-between items-center p-3">
                <span className="font-semibold text-lg md:text-xl">Thiết lập - {chartTitle}</span>
            ...
            <DialogContent className="p-4 md:p-6 bg-gray-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 bg-white p-4 rounded-md border border-gray-200 mb-6 shadow-sm">
                    <div>
                        ...
                        <RadioGroup value={config.timeRange} onChange={(e) => handleChange('timeRange', e.target.value)}>
                            {TIME_OPTIONS.map(opt => (
                                <FormControlLabel key={opt} value={opt} control={<Radio size="small" color="success" />} label={<span className="text-sm">{opt}</span>} className="mb-[-5px]" />
                            ))}
                        </RadioGroup>
                    </div>
                    ...
                    <div className="border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-6">
                        ...
                        <RadioGroup value={config.groupBy} onChange={(e) => handleChange('groupBy', e.target.value)}>
                ...
                <div className="flex justify-between items-center mb-2">
                    ...
                    <Switch checked={config.showDataLabels} onChange={(e) => handleChange('showDataLabels', e.target.checked)} color="success" />
                </div>
                <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-700">Hiển thị chú thích</span>
                    <Switch checked={config.showLegend} onChange={(e) => handleChange('showLegend', e.target.checked)} color="success" />
                </div>
            </DialogContent>

            <DialogActions className="p-4 bg-gray-50 flex flex-col sm:flex-row justify-between border-t border-gray-200 gap-2 sm:gap-0">
                <Button startIcon={<RestoreIcon />} onClick={handleRestore} className="text-gray-700 normal-case font-semibold w-full sm:w-auto">
                    Khôi phục mặc định
                </Button>
                <Button variant="contained" startIcon={<SaveIcon />} onClick={() => onSave(config)} className="bg-[#0ea5e9] hover:bg-blue-600 normal-case rounded-full px-6 w-full sm:w-auto">
                    Lưu
                </Button>
            </DialogActions>
        </Dialog>
