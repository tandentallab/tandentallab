...
function PhieuNhapTable({ data, selectedId, onRowClick }) {
  return (
    <div className="flex flex-col flex-1 min-w-0">
      ...
      <div className="hidden sm:block max-h-[500px] overflow-y-auto table-scroll">
        <table className="w-full border-collapse text-sm text-left bg-white">
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td className={`${rowBase} text-gray-400`} colSpan={5}>
                  Không có dữ liệu
                </td>
              </tr>
            ) : (
              data.map((row) => {
                const nccDaiDien =
                  row.danhSachVatLieu?.[0]?.nhaCungCap?.ten || "—";
                const nccSet = new Set(
                  (row.danhSachVatLieu || [])
                    .map((v) => v.nhaCungCap?.ten)
                    .filter(Boolean)
                );
                ...
                return (
                  ...
                  >
                    <td
                      className={`${rowBase} ${borderBottom} whitespace-nowrap`}
                    >
                      {formatNgay(row.ngayTao)}
                    </td>
                    <td
                      className={`${rowBase} ${borderBottom} whitespace-nowrap`}
                    >
                    ...
                    <td
                      className={`${rowBase} ${borderBottom} max-w-32 truncate`}
                    >
                      {nccDaiDien}
                      {coNhieuNCC && (
                    ...
                    <td className={`${rowBase} ${borderBottom}`}>
                      <span
                        className={`text-sm text-white font-medium px-2.5 py-0.5 ${
      ...
      {/* Cards — chỉ hiện trên mobile */}
      <div className="block sm:hidden">
        <div className="py-1.5 px-2 text-xs font-semibold text-sky-700 bg-sky-100">
          Phiếu nhập
        </div>
        {data.length === 0 ? (
          <p className="px-3 py-4 text-sm text-gray-400">Không có dữ liệu</p>
        ) : (
          data.map((row) => {
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  ...
                  <span
                    className={`text-xs text-white font-medium px-2 py-0.5 rounded shrink-0 ${
                      row.trangThai === "Đã nhận"
                        ? "bg-green-500"
                        : "bg-yellow-500"
                    }`}
        ...
        {data.length > 0 && (
          <div className={`px-3 py-2 text-xs font-medium ${imBg}`}>
            Tổng:{" "}
            {data
              .reduce((s, r) => s + (r.tongTien || 0), 0)
              .toLocaleString("vi-VN")}
...
function PhieuXuatTable({ data, selectedId, onRowClick }) {
  return (
    <div className="flex flex-col flex-1 min-w-0">
      ...
      {/* Cards — chỉ hiện trên mobile */}
      <div className="block sm:hidden">
        <div className="py-1.5 px-2 text-xs font-semibold text-green-700 bg-green-100">
          Phiếu xuất
        </div>
        {data.length === 0 ? (
          <p className="px-3 py-4 text-sm text-gray-400">Không có dữ liệu</p>
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-3 mt-0.5 text-xs text-gray-500">
                  {row.boPhan && <span>🏢 {row.boPhan}</span>}
                  {row.nhanVien && <span>👤 {row.nhanVien}</span>}
                </div>
              </div>
            );
          })
      ...
      <div className="max-h-[300px] overflow-y-auto table-scroll">
        <table className="w-full border-collapse text-sm text-left bg-white">
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td className={`${rowBase} text-gray-400`} colSpan={2}>
                  Không có dữ liệu
                </td>
              </tr>
            ) : (
          ...
          {data.length > 0 && (
            <tfoot className="sticky bottom-0 z-10">
              <tr>
                ...
                <td className={`${rowBase} ${imBg} font-medium`}>
                  {tongSoLuong}
                </td>
              </tr>
            </tfoot>
          ...
          {data.length > 0 && (
            <tfoot className="sticky bottom-0 z-10">
              <tr>
                ...
                <td className={`${rowBase} ${exBg} font-medium`}>
                  {tongSoLuong}
                </td>
              </tr>
            </tfoot>
...
export default function NhapXuatTable() {
  ...
  const { phieuNhapKhos, loading: loadingNhap } = useSelector(
    (state) => state.phieuNhapKho
  );
  const {
    phieuXuatKhos,
    loading: loadingXuat,
  ...
  const isFiltered =
    appliedNCC !== "" || appliedMonth !== "" || appliedBoPhan !== "";
  ...
  const vatLieuNhap = useMemo(
    () => aggregateVatLieu(filteredImportData),
    [filteredImportData]
  );
  const vatLieuXuat = useMemo(
    () => aggregateVatLieu(filteredExportData),
    [filteredExportData]
  );
  ...
  return (
    <div>
      ...
      {/* Toolbar */}
      <div className="mb-3 flex flex-col sm:flex-row gap-2 sm:justify-between sm:items-center rounded">
      ...
      <div className="flex flex-col sm:flex-row w-full gap-0 border border-gray-200 rounded">
        <PhieuNhapTable
